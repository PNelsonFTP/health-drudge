...
function fallback(articles: Article[], ctx: BriefContext | undefined): Brief {
  ...
  const mixOrder = [
    "cholesterol", "apob", "cac", "glucose", "exercise",
    "nutrition", "research", "analysis", "community",
  ];
  if (ctx?.categories) {
  ...
  const headline = cited.length > 0
    ? `Today in cardiometabolic health: ${cited[0].title}`
...
const SYSTEM_PROMPT = `You are the editor of Health Drudge, a cardiometabolic news aggregator
focused on lowering LDL/cholesterol, stabilizing CAC, lowering ApoB, lowering fasting glucose,
exercising more, and eating cleaner. The reader uses this as background while working with their doctors.
...
- Do NOT invent statistics, quotes, names, lab values, or dates that are not in the input.
- Do not prescribe medications, doses, or personal treatment plans.
- Each bullet must be one sentence, ~15-25 words.
- Be neutral and factual. No hype words.
- Mix: include lipids (LDL/ApoB), imaging/CAC, glucose, and lifestyle (exercise or nutrition) when those exist.
