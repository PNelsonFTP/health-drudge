...
export const KEYWORD_AGNOSTIC_SOURCES = new Set([
  "ScienceDaily Cholesterol",
  "ScienceDaily Heart Disease",
  "ScienceDaily Diabetes",
  "ScienceDaily Fitness",
  "ScienceDaily Nutrition",
  "MedicalXpress Cardiology",
  "MedicalXpress Endocrinology & Metabolism",
  "Phys.org Cardiology",
]);

export function isAggregatorSource(name: string): boolean {
  if (KEYWORD_AGNOSTIC_SOURCES.has(name)) return true;
  if (name.startsWith("HN:") || name.startsWith("GN:") || name.startsWith("X.com")) return true;
  if (name.includes("Google News") || name.startsWith("Reddit")) return true;
  if (name.includes("ScienceDaily") || name.includes("MedicalXpress")) return true;
  return false;
}
