...
export function useBookmarks() {
  const { set: bookmarks, toggle, has } = useLocalStorageSet("health-drudge:bookmarks");
  return { bookmarks, toggle, has };
}

export function useReadLater() {
  const { set: queue, toggle, has, remove } = useLocalStorageSet("health-drudge:read-later");
  return { queue, toggle, has, remove };
}

export function useMutedSources() {
  const { set: muted, toggle, has } = useLocalStorageSet("health-drudge:muted-sources");
  return { muted, toggle, has };
}

export function useMutedCategories() {
  const { set: muted, toggle, has } = useLocalStorageSet("health-drudge:muted-categories");
  return { muted, toggle, has };
}

const SNAPSHOT_KEY = "health-drudge:article-snapshots";
